...
- Reviewed implementation tip: `e6d862528652f31f9dd3bf02035613ec9c587146` (`docs: record opponent damage test evidence`)
- Xcode: 26.6 (17F113)
- Swift: 6.3.3
...
`xcresulttool get test-results summary` reports 98 logical tests passed, 0 failed, 0 skipped, and 0 expected failures. The console split is 83 Swift Testing tests across 13 suites plus 15 XCTest UI tests. The device summary records 112 passed executions because four parameterized tests expand into 18 runs.
...
`xcresulttool get build-results` reports status `succeeded`, error count 0, warning count 0, and the selected iPhone 17 Pro / iOS 26.5 destination. The console separately emitted the nonblocking Xcode message that App Intents metadata extraction was skipped because the app has no AppIntents framework dependency.

The UI runner also emitted external simulator/tooling diagnostics about a missing LLDB debugger-version snapshot and duplicate WebCore/WebKit accessibility bundle classes. These messages did not correspond to an app test failure; every test completed and the aggregate command exited 0.
...
| Add through five | `addOpponentAppendsDefaultAndStopsAtFive` and the UI flow | Add Opponent appends one visible zero-damage opponent with a stable UUID/default name, persists it, and stops mutating at five. |
| Tap, hold, exact value, and floor | `repeatStyleDamageChangesAtThresholdAndZeroOnlyPersistRealMutations`, `testOpponentCardsAddDamageAndPersist`, and `testPrimaryDamageExactEntryLethalAndInvalidInput` | Tap changes by one; the integrated repeat path mutates one point at a time; exact whole-number entry works; decrement at zero and same-value exact entry are no-ops; negative and overflowing inputs do not mutate. |
...
| Haptics and no Phase 3A Undo | Integrated store/repeat tests plus UI accessibility interaction | Repeat haptics occur only after a real repeat mutation; tap/hold damage changes do not create the direct-manual-life Undo path. |
| Persistence and relaunch | `testOpponentCardsAddDamageAndPersist`, JSON repository tests, and state round trips | After Opponent 1 damage becomes 1 and linked life becomes 39, termination and relaunch restore both values and the added opponent. |
...
| `CMD-008` | Stable per-opponent primary damage supports independent tap/hold/exact updates, floor zero, persistence, and atomic optional life linking. |
| `CMD-010` | Primary 21+ state uses destructive color plus the semantic `commander lethal` value. Warning banners remain deferred. |
| `CMD-011` | Primary damage acceptance behavior covers delta linking, link-off, exact entry, no-op/floor, overflow rejection, persistence, mutation-only repeat haptics, and no Phase 3A life Undo. |
...
| `OPP-002` | Add Opponent remains on Game, appends through five, uses a visible zeroed default, persists, and hides at maximum. Quick Restart synchronization remains deferred. |
| `OPP-003` | Opponents use UUID identity independent of mutable display names; primary damage targets identity and preserves order. Rename/hide/status/random consumers remain deferred. |
...
| `OPP-005` | One primary commander row exposes direct tap/hold/exact controls, floor zero, linked life, lethal semantic/color treatment, and persistence. Partner/menu/status content is not implemented. |
